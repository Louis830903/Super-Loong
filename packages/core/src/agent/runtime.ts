...
import type { ExecuteToolParams } from "./tool-executor.js";
import {
...
export class AgentRuntime {
   ...
   *
   * B3: 委托到独立模块 tool-executor.ts
   */
  private buildToolDefinitions(): LLMToolDef[] {
    return buildToolDefinitions({
      tools: this.tools,
      providerId: this.config.llmProvider?.providerId ?? "",
      model: this.config.llmProvider?.model ?? "",
      modelDefCache: this.getModelDef(),
    });
  }
  ...
  /** Execute a tool by name. B3: 委托到独立模块 tool-executor.ts */
  private async executeTool(
    ...
    return executeTool({
      ...
      agentId: this.id,
      session,
      configMetadata: this.config.metadata,
    });
  }
  ...
  /**
   * T2: 处理工具执行后的反思钩子。B3: 委托到独立模块 tool-executor.ts
   */
