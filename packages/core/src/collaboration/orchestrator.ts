...
export class GroupChatExecutor extends EventEmitter {
    ...
    try {
      while (turn < config.maxTurns) {
        ...
        const speaker = await this.selectSpeaker(
          ...
          messages,
          speakerIndex,
          turn,    // G-5: 传入轮次用于缓存过期判断
          chatId,  // G-5: 传入 chatId 用于缓存 key
        );
        ...
        // Get speaker's response (F-3: 带重试)
        ...
        if (lastTurnError || !response) {
          ...
          // G-5/M5: 重试全部失败后，从发言者缓存中移除该故障 agent
        ...
        // A-2: 使用循环外预构建的 allConditions（不再每轮重复构建）
      ...
      // 聚合所有消息中的附件（去重），无附件时不设置字段
      const chatAttachments = deduplicateAttachments(
      ...
      // F-1: 清理 lastSpeakerByChat 缓存，防止内存泄漏
  ...
  private async selectSpeaker(
    ...
    messages: CollabMessage[],
    currentIndex: number,
    turn?: number,   // G-5: 当前轮次
    chatId?: string, // G-5: 缓存 key
    ...
    switch (config.speakerSelection) {
      ...
      case "auto": {
        // G-5: 带缓存的 LLM-based speaker selection
        return this.autoSelectSpeaker(config, participants, messages, turn ?? 0, chatId ?? "default");
      }

      case "manual": {
        // H-2: 真正的手动指定发言顺序 — 按 manualSpeakerOrder 循环
        if (!config.manualSpeakerOrder?.length) {
          // 未指定顺序时 fallback 为 round_robin
          return participants[currentIndex % participants.length];
        }
        const idx = ((turn ?? 1) - 1) % config.manualSpeakerOrder.length;
        const targetId = config.manualSpeakerOrder[idx];
        return participants.find((p) => p.id === targetId) ?? participants[0];
      }

      case "graph": {
        // I-4: 状态机发言选择 — 按 transitions 邻接表转移
        const gc = config.graphConfig;
        if (!gc) return participants[0]; // 无 graphConfig fallback
        ...
        // 找到上一轮发言者
        const lastMsg = [...messages].reverse().find((m) => m.role === "chat" && m.agentId !== "user" && m.agentId !== "system");
        ...
        // 多个后继 → 使用 LLM 从缩小的候选集中选择（M1: 不走缓存）
        return this.graphSelectFromCandidates(config, participants, messages, successors, chatId ?? "default");
      }

      case "handoff": {
        // I-4: Agent 自主交接 — 解析上一条消息中的 [HANDOFF:name] 指令
        if (messages.length <= 1) {
          // 第一轮 fallback round_robin
          return participants[currentIndex % participants.length];
        }

        const lastChatMsg = [...messages].reverse().find((m) => m.role === "chat" && m.agentId !== "user" && m.agentId !== "system");
        if (lastChatMsg) {
          const handoffPattern = /\[HANDOFF:([^\]]+)\]/;
          const match = lastChatMsg.content.match(handoffPattern);
          if (match) {
        ...
        // 无 handoff 指令 → fallback round_robin
        return participants[currentIndex % participants.length];
      }
...
Only output the JSON array, nothing else.`;
      ...
      try {
        const { response } = await withTimeout(
          moderator.chat(selectionPrompt, `speaker_select_${chatKey}`),
          30_000,
          "Speaker selection",
        );
        ...
        if (suggestions && suggestions.length > 0) {
          ...
          if (resolvedIds.length > 0) {
            ...
            // 缓存剩余建议
            this.speakerSuggestionCache.set(chatKey, {
              queue: resolvedIds,
              lastRefresh: turn,
            });
  ...
  invalidateSpeakerCache(chatKey: string, agentId: string): void {
    const cached = this.speakerSuggestionCache.get(chatKey);
    if (cached) {
      cached.queue = cached.queue.filter((id) => id !== agentId);
    }
  }
  ...
  private async graphSelectFromCandidates(
    ...
    const candidateList = candidateIds
      .map((id) => {
        ...
        return p ? `- ${p.state.config.name} (${p.id})` : null;
      })
      .filter(Boolean)
      .join("\n");
...
Respond with the exact agent name only.`;
    ...
    try {
      const { response } = await withTimeout(
        moderator.chat(prompt, `graph_select_${chatId}`),
        15_000,
        "Graph speaker selection",
      );
      ...
      return match ?? participants.find((p) => candidateIds.includes(p.id)) ?? participants[0];
    } catch {
      // LLM 失败 → 选第一个候选
